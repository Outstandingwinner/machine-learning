...
def svdEst(dataMat, user, simMeas, item):
    ...
    print "&&&&&&&&&&&&&&&&&&&&&&&&"
    print "shape(dataMat.T)",shape(dataMat.T)
    print "shape(U[:,:4])",shape(U[:,:4])
    print "shape(Sig4.I)",shape(Sig4.I)
    print "Sig4=",Sig4
    print "Sig4.I=",Sig4.I
    for j in range(n):
        userRating = dataMat[user,j]
...
def recommend(dataMat, user, N=3, simMeas=cosSim, estMethod=standEst):
    ...
    print "nonzero(dataMat[user,:].A==0)=",nonzero(dataMat[user,:].A==0)
    ...
    print "sorted(itemScores, key=lambda jj: jj[1], reverse=True)[:N]=",sorted(itemScores, key=lambda jj: jj[1], reverse=True)[:N]
