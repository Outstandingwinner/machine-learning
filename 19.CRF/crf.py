...
class CRF:
    def __init__(self, feature_functions, labels):
        ...
        print("*****************")
        print("len(self.ft_fun)=",len(self.ft_fun))
    ...
    def create_vector_list(self, x_vecs, y_vecs):
        ...
        print("np.shape(observations)=",np.shape(np.mat(observations)))
        print("labels=",labels)
    ...
    def neg_likelihood_and_deriv(self, x_vec_list, y_vec_list, w, debug=False):
        ...
        for x_vec, y_vec in zip(x_vec_list, y_vec_list):
            ...
            print("===========================")
            print("np.shape(all_features)",np.shape(all_features))
            print("np.shape(w)",np.shape(w))
            # np.shape(all_features)(37, 21, 21, 2354)
            # np.shape(w)(2354, )
            # np.shape(log_M_s)(37, 21, 21)
            log_M_s = np.dot(all_features, w)
            print("np.shape(log_M_s)", np.shape(log_M_s))
