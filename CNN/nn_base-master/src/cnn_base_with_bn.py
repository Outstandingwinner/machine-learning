...
class BNLayer(object):
    ...
    def __init__(self,LName,eps,miniBatchesSize,channel,i_size,activator,optimizer, dataType):
        self.name = LName
        self.miniBatchesSize = miniBatchesSize
        self.i_size = i_size
        self.channel = channel
    ...
    def bp(self,input,delta,lrt):
        ...
        gammaNew, betaNew = self.optimizer.getUpdWeights(self.gamma, dgamma, self.beta, dbeta, lrt)
        self.gamma = gammaNew
        self.beta = betaNew
...
class Session(object):
    ...
    def train_steps(self, train_data, y_, lrt):
        acc, loss, delta = self.inference(train_data, y_ , val=False)
        self.bp(delta, lrt)
        return acc, loss

    # 独立数据集验证训练结果
    def validation(self, data_v, y_v):
        acc, loss, _ = self.inference(data_v, y_v, val=True)
        return acc, loss
...
def main():
    ...
    conv1 = ConvLayer('conv1', Params.MINI_BATCH_SIZE, Params.IMAGE_SIZE, Params.IMAGE_CHANNEL,
                      Params.CONV1_F_SIZE, Params.CONV1_O_DEPTH,
                      Params.CONV1_O_SIZE, Params.CONV1_STRIDES,
                      NoAct, conv1Optimizer, Params.DTYPE_DEFAULT)
    #在BN后再激活
    bn1Optimizer = AdmOptimizer(Params.BETA1, Params.BETA2, Params.EPS, Params.DTYPE_DEFAULT)
    bn1 = BNLayer('bn1',Params.EPS,Params.MINI_BATCH_SIZE,Params.CONV1_O_DEPTH,
                      Params.CONV1_O_SIZE,ReLU,bn1Optimizer, Params.DTYPE_DEFAULT)
    ...
    bn2Optimizer = AdmOptimizer(Params.BETA1, Params.BETA2, Params.EPS, Params.DTYPE_DEFAULT)
    bn2 = BNLayer('bn2',Params.EPS,Params.MINI_BATCH_SIZE,Params.CONV2_O_DEPTH,
                      Params.CONV2_O_SIZE,ReLU,bn2Optimizer, Params.DTYPE_DEFAULT)
